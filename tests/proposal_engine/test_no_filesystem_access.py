"""Guardrail: the proposal_engine package must never touch the filesystem.

Same strict shape as the classifier's guardrail (FA-005) — proposal
generation is a pure, total function over an already-in-memory
ClassificationResult, so NO filesystem I/O of any kind is expected: any
open(), any os.*/shutil.*/io.* call, and any pathlib.Path I/O method,
anywhere in src/file_agent/proposal_engine/.
"""
...
PROPOSAL_ENGINE_DIR = (
    Path(__file__).resolve().parents[2] / "src" / "file_agent" / "proposal_engine"
)
...
def test_no_proposal_engine_source_file_touches_the_filesystem() -> None:
    source_files = sorted(PROPOSAL_ENGINE_DIR.glob("*.py"))
    assert source_files, (
        f"expected proposal_engine source files under {PROPOSAL_ENGINE_DIR}"
    )
