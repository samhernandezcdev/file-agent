"""Guardrail: the scanner package must never call filesystem-mutating APIs.

AST-based (not substring) per FA-001 review m2, which flagged the domain
layer's substring-based guardrail as too narrow and prone to false
positives/negatives (e.g. `.replace(` matching unrelated string methods).
"""
...
SCANNER_DIR = Path(__file__).resolve().parents[2] / "src" / "file_agent" / "scanner"
...
def test_no_scanner_source_file_mutates_the_filesystem() -> None:
    scanner_files = sorted(SCANNER_DIR.glob("*.py"))
    assert scanner_files, f"expected scanner source files under {SCANNER_DIR}"
